...
export function PromptsModal({ open, onClose }: PromptsModalProps) {
  ...
  const [messageById, setMessageById] = useState<Record<string, { text: string; type: 'ok' | 'err' }>>({});
  ...
  const categories = useMemo(() => Array.from(new Set(prompts.map((item) => item.category))), [prompts]);
  ...
  const resetPrompt = async (id: string) => {
    if (!window.confirm('¿Restaurar este prompt al texto original?\nSe perderán los cambios personalizados.')) {
      return;
    }
      ...
      >
          ...
          {categories.map((category) => (
            <button
              ...
              className={`border-b-2 px-4 py-3 text-sm font-semibold transition ${
                category === activeCategory ? 'border-accent text-accent' : 'border-transparent text-dim hover:text-text'
              }`}
            >
              {category}
            </button>
        ...
        <div className="px-6 py-5">
          <div className="mb-4 rounded-lg border border-accent/20 bg-accent/5 p-3 text-xs text-dim">
            Este panel cambia el comportamiento real del pipeline. Antes de editar, revisa <b>Qué hace</b>, <b>Cuándo se usa</b> y{' '}
            <b>Si lo cambias</b>. Las llaves dobles <code>{'{{ }}'}</code> son literales. Las llaves simples{' '}
            <code>{'{variable}'}</code> se reemplazan con datos reales en ejecución.
          </div>
            ...
            return (
              <article key={prompt.id} className="mb-4 rounded-xl border border-border bg-code p-4">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
                      prompt.type === 'meta' ? 'bg-purple/20 text-purple' : 'bg-orange/20 text-orange'
                    }`}
                  >
                    {prompt.type}
                  </span>
                ...
                <div className="mb-3 space-y-1 rounded-lg border border-border bg-white/5 p-3 text-xs text-text">
                  <p>
                    <span className="font-bold text-accent">Qué hace:</span> {prompt.what_it_does || 'Sin descripción'}
                  </p>
                  <p>
                    <span className="font-bold text-accent">Cuándo se usa:</span> {prompt.when_it_runs || 'Sin descripción'}
                  </p>
                  <p>
                    <span className="font-bold text-accent">Si lo cambias:</span> {prompt.if_you_change_it || 'Sin descripción'}
                  </p>
                  <p>
                    <span className="font-bold text-accent">Riesgo al tocarlo:</span>{' '}
                    <span className={riskLabelClass(prompt.risk_level)}>{prompt.risk_level || 'no definido'}</span>
                  </p>
                </div>
                ...
                {prompt.variables.length > 0 ? (
                  <div className="mb-3 flex flex-wrap gap-1">
                    {prompt.variables.map((variable) => (
                      <span key={variable} className="rounded-md border border-accent/30 bg-accent/15 px-2 py-1 font-mono text-[11px] text-accent">
                        {`{${variable}}`}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <span className={`ml-auto text-xs ${msg?.type === 'ok' ? 'text-green' : 'text-red'}`}>{msg?.text || ''}</span>
                </div>
              </article>
            );
          })}
